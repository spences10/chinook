<script lang="ts">
	...
	const data = $derived(await get_track(Number(page.params.id)));
...
{#if data}
	<div class="container mx-auto p-8">
		...
		<div class="mb-8 space-y-1 text-muted-foreground">
			<p>
				by <a href="/artists/{data.ArtistId}" class="hover:underline"
					>{data.ArtistName}</a
				>
			</p>
			<p>
				from <a href="/albums/{data.AlbumId}" class="hover:underline">
					{data.AlbumTitle}
				</a>
			</p>
		</div>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<div class="rounded-lg border p-4">
				...
				<div class="text-xl font-medium">
					{formatDuration(data.Milliseconds)}
				</div>
			</div>
			<div class="rounded-lg border p-4">
